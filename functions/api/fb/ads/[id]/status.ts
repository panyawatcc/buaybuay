...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    // FB API: use URL params for ad status update (not JSON body)
    const fbUrl = new URL(`https://graph.facebook.com/v25.0/${adId}`);
    ...
    return Response.json({ success: true, id: adId, status: body.status });
  } catch {
